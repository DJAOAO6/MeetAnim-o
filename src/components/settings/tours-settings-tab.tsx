...
import { deleteTourAction, deleteZoneAction, reassignAndDeleteZoneAction, saveTourAction, saveZoneAction, toggleTourStatusAction } from "@/lib/tours-actions";
import { deleteSavedPlaceAction, updateTourPreferencesAction, upsertSavedPlaceAction } from "@/lib/tour-runs-actions";
import { recurrenceMentions } from "@/lib/tour-schedule";
...
// "Chaque jeudi, 9h-16h30" plutôt que "Toutes les semaines" en toutes
// lettres — recurrenceMentions (tour-runs.ts) déjà utilisée par la liste de
// journées, réutilisée telle quelle plutôt que reformulée ici.
function formatTourSchedule(tour: Tour): string {
  const dayPart = tour.recurrence === "Une seule fois"
    ? (tour.dateId ?? tour.day.toLocaleLowerCase("fr-FR"))
    : (recurrenceMentions[tour.recurrence]?.(tour.day) ?? tour.day.toLocaleLowerCase("fr-FR"));
...
export function ToursSettingsTab({ initialTours, initialZones, initialSavedPlaces, initialPreferences, cabinetAvailable, upcomingGeneratedCounts }: ToursSettingsTabProps) {
  ...
  const [tourModal, setTourModal] = useState<{ open: boolean; tour?: Tour }>({ open: false });
  const [zonesPanelOpen, setZonesPanelOpen] = useState(false);
  const [zoneModal, setZoneModal] = useState<{ open: boolean; zone?: Zone } | null>(null);

  function zoneNamesFor(tour: Tour): string {
    const names = tour.zoneIds.map((id) => zones.find((zone) => zone.id === id)?.name).filter((name): name is string => Boolean(name));
    return names.length > 0 ? names.join(", ") : "aucune zone";
  }

  async function handleSaveTour(value: TourFormValue) {
    const result = await saveTourAction(value);
    if (!result.ok) {
      notify.error(result.error);
      return;
    }
    setTours((current) => (current.some((item) => item.id === result.tour.id) ? current.map((item) => (item.id === result.tour.id ? result.tour : item)) : [...current, result.tour]));
    notify.success(value.id ? "Tournée modifiée." : "Tournée créée.");
    setTourModal({ open: false });
    // Une tournée créée/modifiée peut changer les journées déjà générées
    // (nouveau motif actif, jour/horaire changé) — router.refresh() les
    // recompte (getUpcomingGeneratedCounts, chargée par la page serveur).
    router.refresh();
  }
  ...
  async function handleSaveZone(value: ZoneFormValue) {
    const result = await saveZoneAction(value);
    if (!result.ok) {
      notify.error(result.error);
      return;
    }
    setZones((current) => (current.some((item) => item.id === result.zone.id) ? current.map((item) => (item.id === result.zone.id ? result.zone : item)) : [...current, result.zone]));
    notify.success(value.id ? "Zone modifiée." : "Zone créée.");
    setZoneModal(null);
  }
  ...
  async function handleReassignAndDelete(zoneId: string, targetZoneId: string) {
    ...
    setTours((current) => current.map((tour) => (tour.zoneIds.includes(zoneId) ? { ...tour, zoneIds: tour.zoneIds.map((id) => (id === zoneId ? targetZoneId : id)) } : tour)));
    notify.success("Zone réassignée puis supprimée.");
  }
    ...
    <div className="space-y-10">
      <div>
        ...
        <div className="space-y-3">
          {tours.length === 0 ? (
            <p className="text-sm text-animeo-muted">Aucune tournée récurrente pour l’instant.</p>
          ) : (
            tours.map((tour) => {
              const generatedCount = upcomingGeneratedCounts[tour.id] ?? 0;
              return (
                <Card key={tour.id} className="p-5 sm:p-6">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <h3 className="text-lg font-black text-animeo-dark">{tour.name}</h3>
        ...
        <p className="mt-5 rounded-2xl bg-animeo-soft p-4 text-sm text-animeo-dark">
          <strong>Rappel :</strong> une zone regroupe des villes et codes postaux ; une tournée récurrente associe une ou plusieurs zones à un rythme et des horaires — les journées datées se posent ensuite automatiquement.{" "}
          <Link href="/dashboard/tournees" className="font-extrabold underline">Voir mes journées de tournée →</Link>
        </p>
      </div>
      ...
      {tourModal.open ? (
        <TourModal
          ...
          onDelete={tourModal.tour ? handleDeleteTour : undefined}
        />
      ) : null}
      ...
      {zonesPanelOpen ? (
        <ZonesPanel
          ...
          onNewZone={() => setZoneModal({ open: true })}
      ...
      {zoneModal?.open ? <ZoneModal zone={zoneModal.zone} onClose={() => setZoneModal(null)} onSave={handleSaveZone} /> : null}
    </div>
  );
}
