...
export function TourRunOptimizeModal({ comparison, applying, onApply, onDismiss }: TourRunOptimizeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-animeo-deep/60 p-4 backdrop-blur-sm" role="presentation">
      <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="optimize-title" className="w-full max-w-md rounded-[18px] bg-white shadow-[0_24px_70px_rgb(var(--theme-shadow-rgb)/0.3)] outline-none">
        <div className="p-6">
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-animeo-border-soft p-5 sm:flex-row sm:justify-end">
          <button type="button" onClick={onDismiss} className="rounded-xl border border-animeo-border px-5 py-2.5 text-sm font-extrabold text-animeo-dark transition hover:bg-animeo-bg">Garder ma tournée</button>
          <button type="button" onClick={onApply} disabled={applying} className="rounded-xl bg-animeo px-5 py-2.5 text-sm font-extrabold text-white transition hover:bg-animeo-hover disabled:cursor-not-allowed disabled:opacity-60">
