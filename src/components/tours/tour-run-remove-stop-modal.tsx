...
export function TourRunRemoveStopModal({ stopLabel, submitting, onRemoveFromTour, onCancelAppointment, onClose }: TourRunRemoveStopModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-animeo-deep/60 p-4 backdrop-blur-sm" role="presentation">
      <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="remove-stop-title" aria-describedby="remove-stop-message" className="w-full max-w-sm rounded-[18px] bg-white shadow-[0_24px_70px_rgb(var(--theme-shadow-rgb)/0.3)] outline-none">
        <div className="p-6">
        ...
        <div className="flex flex-col gap-2 border-t border-animeo-border-soft p-5">
          <button type="button" onClick={onRemoveFromTour} disabled={submitting} className="rounded-xl bg-animeo px-5 py-2.5 text-sm font-extrabold text-white transition hover:bg-animeo-hover disabled:cursor-not-allowed disabled:opacity-60">
            Retirer de la tournée (garder le rendez-vous)
          </button>
          <button type="button" onClick={onCancelAppointment} disabled={submitting} className="rounded-xl bg-animeo-error px-5 py-2.5 text-sm font-extrabold text-white transition hover:bg-animeo-danger disabled:cursor-not-allowed disabled:opacity-60">
            Annuler le rendez-vous
          </button>
          <button type="button" onClick={onClose} disabled={submitting} className="rounded-xl border border-animeo-border px-5 py-2.5 text-sm font-extrabold text-animeo-dark transition hover:bg-animeo-bg disabled:cursor-not-allowed disabled:opacity-60">
            Ne rien faire
          </button>
        </div>
      </section>
