...
export function ReminderScheduleModal({ reminder, clients, onClose, onSave }: ReminderScheduleModalProps) {
  ...
  const [dueDate, setDueDate] = useState(reminder?.dueDate ?? "2026-09-24");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#102f37]/55 p-4 backdrop-blur-sm" role="presentation">
      <section role="dialog" aria-modal="true" aria-labelledby="schedule-dialog-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/20 bg-white shadow-[0_24px_70px_rgba(12,39,47,0.3)]">
        <div className="flex items-start justify-between border-b border-[#e5eeeb] bg-gradient-to-r from-animeo-soft to-white p-5 sm:p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-animeo-muted">Les informations sont conservées uniquement jusqu’à l’actualisation.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
            ...
            <div className="sm:col-span-2 rounded-2xl border border-[#cfe7e1] bg-animeo-soft p-4 text-xs font-semibold leading-relaxed text-animeo-dark">
              Si la date choisie est postérieure au 24 août 2026, le rappel sera classé <strong>À venir</strong>. Il ne passera pas prématurément dans <strong>À relancer</strong>.
            </div>
          </div>
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-[#e5eeeb] p-5 sm:flex-row sm:justify-end sm:p-6">
            ...
            <button type="submit" className="rounded-xl bg-animeo px-5 py-2.5 text-sm font-extrabold text-white transition hover:bg-[#459e90]">
              {reminder ? "Enregistrer les modifications" : "Programmer le rappel"}
            </button>
          </div>
        </form>
      </section>
