...
export const bookingErrorInputClassName = "border-animeo-danger-border focus:border-animeo-danger focus:ring-animeo-danger/15";
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between gap-2 text-sm font-extrabold text-animeo-dark">
        <label htmlFor={id}>
          {label}
          {required ? <span className="ml-1 text-animeo-danger" aria-hidden="true">*</span> : null}
        </label>
        {error ? (
          <span id={`${id}-error`} className="flex items-center gap-1 text-xs font-bold text-animeo-danger" role="alert">
